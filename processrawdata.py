...
if __name__ == "__main__":
    ...
    skill_df = pd.read_csv(SKILL_BUILDER_DATA, header=0, index_col=None, encoding='utf8', dtype=dtype_dict,
                           usecols=['order_id', 'user_id', 'problem_id', 'skill_id', 'correct'])
    nskill_df = pd.read_csv(NON_SKILL_BUILDER_DATA, header=0, encoding='utf8', dtype=dtype_dict,
                            usecols=['order_id', 'user_id', 'problem_id', 'skill_id', 'correct'])
    ...
    timestamp = dt.now()
    with open('ASSISTment_{}_train_{:02d}{:02d}.txt'.format(option, timestamp.month, timestamp.day), 'w',
              encoding='utf8') as f:
        train_txt = get_txt_from_data_dict(train_dict)
        f.write(train_txt)
    with open('ASSISTment_{}_test_{:02d}{:02d}.txt'.format(option, timestamp.month, timestamp.day), 'w',
              encoding='utf8') as f:
